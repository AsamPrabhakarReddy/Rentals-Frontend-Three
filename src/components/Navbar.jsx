...
const Navbar = () => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-20 transition-all duration-300 ease-in-out select-none ${isScrolled ? "bg-white shadow-md py-6" : "bg-white py-6"
        }`}
    >
      <div className="max-w-[1400px] mx-auto flex justify-between items-center px-4">
        ...
        <a href="/" className="flex items-center">
          <div
            className={`h-12 w-12 bg-mainColor rounded-full flex items-center justify-center text-white font-bold transition-all duration-300 ease-in-out ${isScrolled ? "w-12 h-12 rounded-full" : "w-10 h-10 rounded-full"
              }`}
          >
            RMA
          </div>
          ...
          <span
            className={`text-2xl font-semibold ml-1 transition-all duration-300 ease-in-out text-black ${isScrolled ? " text-black" : ""
              }`}
          >
            Housing.
          </span>
        ...
        <ul className="hidden lg:flex flex-grow justify-center space-x-8 items-center">
          <li
            className={` group transition font-normal duration-300 ${isScrolled
              ? "text-footerHeading hover:text-colorFour transition ease-in-out duration-500"
              : "text-navGray hover:text-colorFour transition ease-in-out duration-500"
              }`}
          >
            <a href="/">Home</a>
            <div
              className={` h-[2px] w-0 group-hover:w-full transition-all duration-300 ${isScrolled ? "bg-mainColor" : "bg-mainColor"
                }`}
            ></div>
          </li>
        ...
        <div className="hidden lg:flex space-x-4">
          <a
            ...
            className={`border-[1px] relative font-medium  lg:px-6 md:px-4 py-2 rounded-full text-sm  transition duration-500 ${isScrolled
        ...
        <div className="lg:hidden">
          <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}>
            {isMobileMenuOpen ? (
              <AiOutlineClose
                className={`w-5 h-5 ${isScrolled ? "text-footerLinks" : "text-navGray"
                  }`}
              />
            ) : (
              <RiMenu3Fill
